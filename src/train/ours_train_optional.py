"""
Ours training loop (optional): Audio→Text reverse KL + text→text FKL anchoring.

Adds a text-student distillation arm to preserve text-only accuracy (acc_text)
while the audio-student keeps learning from the privileged text teacher.
"""
...
DEFAULT_OUTPUT_DIR = "Data/ckpts/ours_qwen25_omni_7b_v6_textkd"
DEFAULT_PHI4_MODEL_DIR = "microsoft/Phi-4-multimodal-instruct"
DEFAULT_PHI4_OUTPUT_DIR = "Data/ckpts/ours_phi4_mm_v6_textkd"
...
FROZEN_OUTPUT_DIR_NAMES = {
    ...
    "ours_phi4_mm_v5_brief": "",
}


@dataclass
class PreparedMicrobatch:
    """CPU/GPU-ready micro-batch. R fills cnt+text+para encodings. P fills para.
    ...
    text_cnt_prompt = student public text prompt for the text→text FKL arm.
    """
    ...
    text_cnt_prompt: dict[str, Any] | None = None
...
def compute_loss_text_cnt_batch(
    student: Any,
    prepared: PreparedMicrobatch,
    *,
    max_new_tokens: int,
    ...
    """Offline forward KL on frozen text teacher, forced onto text student.

    Both teacher and student are fed the same privileged text prompt used by
    the audio-cnt teacher. The student arm uses the public text prompt as its
    input; this mirrors the frozen acc_text eval and keeps the text and audio
    content arms aligned on the same concise teacher distribution.
    """

    if prepared.text_cnt_prompt is None or prepared.teacher_prompt is None:
        raise ValueError("R micro-batch missing text-cnt or teacher prompts.")
    student_prompt = prepared.text_cnt_prompt
    teacher_prompt = prepared.teacher_prompt
    ...
    texts = decode_response_ids(tokenizer, response_ids, pad_token_id=pad_token_id)
...
def prepare_microbatch_on_cpu(
    ...
    # v6: text-student sees the same public text question used by acc_text eval.
    text_cnt_convs = [
        build_text_user_conversation(sample.text_prompt) for sample in samples
    ]
    return PreparedMicrobatch(
        layer="R",
        samples=samples,
        ...
        prompt=prompt,
        teacher_para=teacher_para,
        text_cnt_prompt=encode_conversations(processor, text_cnt_convs, device=cpu),
    )
...
def materialize_microbatch_on_device(
    ...
    *,
    ...
    return PreparedMicrobatch(
        ...
        text_cnt_prompt=(
            None
            if prepared.text_cnt_prompt is None
            else _move_batch_to_device(
                prepared.text_cnt_prompt, device=device, dtype=dtype
            )
        ),
    )
...
def _log_micro_rows(
    *,
    ...
    loss_text_cnt_value: float | None,
    loss2_value: float,
    lengths_cnt: list[int],
    lengths_text_cnt: list[int],
    lengths_para: list[int],
    para_texts: list[str],
    text_cnt_texts: list[str],
    ...
    for index, sample in enumerate(prepared.samples):
        text_cnt_preview = text_cnt_texts[index][:200] if text_cnt_texts else ""
        _append_jsonl(
            log_path,
            {
                "epoch": epoch,
                ...
                "slot_loss": slot_loss,
                "loss1": loss1_value,
                "loss_text_cnt": loss_text_cnt_value,
                "loss2": loss2_value,
                "response_tokens_cnt": (
                    lengths_cnt[index] if prepared.layer == "R" else None
                ),
                "response_tokens_text_cnt": (
                    lengths_text_cnt[index]
                    if prepared.layer == "R" and lengths_text_cnt
                    else None
                ),
                "response_tokens_para": lengths_para[index],
                "para_text": para_texts[index],
                "text_cnt_text_preview": text_cnt_preview,
        ...
        if prepared.layer == "R":
            ...
            _append_jsonl(
                length_stats_path,
                {
                    "layer": "R",
                    "arm": "r_text_cnt",
                    "response_tokens": lengths_text_cnt[index],
                    "max_new_tokens": max_new_tokens_r,
                    "optimizer_step": optimizer_step,
                },
            )
...
def run_ours_training(
    *,
    ...
    """Run Ours v6 loop and save epoch-end LoRA adapters."""

    _refuse_ablation_overwrite(output_dir)
    if max_new_tokens_r != 512:
        raise ValueError("v6_textkd R-cnt cap is locked to 512 to match frozen eval.")
    ...
    expected_mass_text_cnt = sampler.prob_r * weight_text_cnt
    print(
        "[ours] 4-arm weights "
        f"r_cnt={weight_r_cnt:.4f} r_text_cnt={weight_text_cnt:.4f} "
        f"r_para={weight_r_para:.4f} p_para={weight_p_para:.4f} "
        f"E[mass]={arm_weights['expected_mass_r_cnt']:.3f}:"
        f"{expected_mass_text_cnt:.3f}:"
    ...
    try:
        for epoch in range(epochs):
            ...
            for _ in range(steps_per_epoch):
                ...
                step_loss_text_cnt: list[float] = []
                step_loss2: list[float] = []
                step_len_cnt: list[int] = []
                step_len_text_cnt: list[int] = []
                ...
                for _acc in range(grad_accum):
                    ...
                    loss_text_cnt_value: float | None = None
                    if prepared.layer == "R":
                        n_r_slots += 1
                        ...
                        loss_text_cnt, lengths_text_cnt, text_cnt_texts = (
                            compute_loss_text_cnt_batch(
                                student,
                                prepared,
                                max_new_tokens=max_new_tokens_r,
                                top_k=fkl_top_k,
                        ...
                        loss_text_cnt_value = float(loss_text_cnt.detach().item())
                        step_loss_text_cnt.append(loss_text_cnt_value)
                        step_len_text_cnt.extend(lengths_text_cnt)
                        (
                            (loss_text_cnt * weight_text_cnt) / grad_accum
                        ).backward()
                        del loss_text_cnt
                    else:
                        n_p_slots += 1
                    ...
                    if prepared.layer == "R":
                        if loss1_value is None or loss_text_cnt_value is None:
                            raise RuntimeError("R slot missing loss1 or loss_text_cnt.")
                        if n_kept > 0:
                            ((loss2 * weight_r_para) / grad_accum).backward()
                        slot_loss = (
                            weight_r_cnt * loss1_value
                            + weight_text_cnt * loss_text_cnt_value
                    ...
                    _log_micro_rows(
                        ...
                        loss_text_cnt_value=loss_text_cnt_value,
                        loss2_value=loss2_value,
                        lengths_cnt=lengths_cnt,
                        lengths_text_cnt=lengths_text_cnt,
                        lengths_para=lengths_para,
                        para_texts=para_texts,
                        text_cnt_texts=text_cnt_texts,
                        micro_batch_size=micro_batch_size,
                ...
                mean_loss_text_cnt = (
                    sum(step_loss_text_cnt) / len(step_loss_text_cnt)
                    if step_loss_text_cnt
                    else None
                )
                mean_loss2 = (
                ...
                mean_len_text_cnt = (
                    sum(step_len_text_cnt) / len(step_len_text_cnt)
                    if step_len_text_cnt
                    else None
                )
                mean_len_para = (
                ...
                mass_r_text_cnt = n_r_slots * weight_text_cnt
                ...
                print(
                    ...
                    f"loss1t={_fmt_opt(mean_loss_text_cnt, digits=4)} "
                    f"loss2={_fmt_opt(mean_loss2, digits=4)} "
                    f"len_cnt={_fmt_opt(mean_len_cnt, digits=1)} "
                    f"len_text_cnt={_fmt_opt(mean_len_text_cnt, digits=1)} "
                    ...
                    f"mass={mass_r_cnt:.1f}:{mass_r_text_cnt:.1f}:"
                    f"{mass_r_para:.1f}:{mass_p_para:.1f} "
    ...
    summary = {
        ...
        "recipe": "v6_textkd",
        "practical_arms": ["R-cnt", "R-cnt-text", "R-para", "P-para"],
        "skipped_arm": "P-cnt",
        "arm_loss_target_ratio": "R-cnt:R-cnt-text:R-para:P-para = 2:1:1:1",
        "arm_loss_weights": {
            **arm_weights,
            "r_cnt_text": weight_text_cnt,
        },
        "epochs": epochs,
        "micro_batch_size": micro_batch_size,
...
def _assert_eval_aligned_conversation(sample: OursTrainSample) -> None:
    ...
    if sample.layer == "R":
        text_cnt_chat = build_text_user_conversation(sample.text_prompt)
        text_cnt_types = [part["type"] for part in text_cnt_chat[0]["content"]]
        if text_cnt_types != ["text"]:
            raise ValueError(
                f"Text-cnt chat must be text-only, got {text_cnt_types}"
            )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Train Ours v6 on Qwen2.5-Omni-7B or Phi-4-MM")
